...
variable "repository" { default = "boshua" }
variable "owner" { default = "dpb587" }

terraform {
  backend "http" {
    # lpass-http-note-proxy "Secure Notes/github.com/dpb587/boshua/terraform.tfstate"
    address = "http://127.0.0.1:22808/note"
  }
}
...
resource "aws_s3_bucket" "bucket" {
  ...
  logging {
    ...
    target_prefix = "${var.owner}-${var.repository}-${var.region}/"
  }
  versioning {
    enabled = true
  }
